...
// BitonicSortUSM
void ParallelBitonicSort(int data_usm[], int n, queue &q) {
  ...
  for (int step = 0; step < n; step++) {
    ...
    for (int stage = step; stage >= 0; stage--) {
      ...
      // int two_power = pow(2, (step-stage));
      int two_power = 1 << (step - stage); //Maneira mais rápida de fazer

      // Offload the work to kernel.
      q.submit([&](auto &h){ // Queue is a connection to an accelerator device.
    ...
    for (int stage = step; stage >= 0; stage--) {
      ...
      q.submit([&](auto &h){
        accessor a(input, h);
          ...
          //i sendo 7 e a seq_len sendo 2 da negativo
          if (i < (seq_len * seq_num) + h_len) swapped_ele = i + h_len; //Entender melhor isso
...
// Function sorts an array in serial using bitonic sort algorithm. The size of the array is indicated by the exponent n: the array size is 2 ** n.
...
// Function in case the exponent is not valid
...
int main(int argc, char *argv[]) {
  int n, seed, size;
  int exp_max = log2(numeric_limits<int>::max()); // Biggest value of exponent

  // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= Read parameters =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  try {
    n = stoi(argv[1]); // Get the value of the exponent

    // Verify the boundary of acceptance. Verify if the exponent is valid
    if (n < 0 || n >= exp_max) {
      Usage(argv[0], exp_max);
      return -1;  
    }

    seed = stoi(argv[2]); // Seed recive the second parameters argument 
    size = pow(2, n); // Size of the main array's
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= MEMORY ALLOCATION =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  int *data_cpu = (int *)malloc(size * sizeof(int)); //Memoria para o array

  // Memory allocated for host access only recursive
  int *data_rec = (int *)malloc(size * sizeof(int));

  // USM allocation using malloc_shared: data stores a sequence of random numbers.
  // USM: Universal Storage Media
  int *data_usm = malloc_shared<int>(size, q); //Vetor com uma sequencia aleatoria de numeros
  ...
  srand(seed);
  for (int i = 0; i < size; i++)//Starting the arrays
    data_usm[i] = data_gpu[i] = data_cpu[i] = data_rec[i] = rand() % 1000;

// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= ParallelBitonicSortUSM =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
#if DEBUG
  cout << "\ndata before:\n";
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= BitonicSortSerialRecursive =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  // Starter timer
  dpc_common::TimeInterval t_serRec;

  bitonicSort(data_rec, 0, size, 1);

  cout << "CPU serial recursive time: " << t_serRec.Elapsed() << " sec\n";
